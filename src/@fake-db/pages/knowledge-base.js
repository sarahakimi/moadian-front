...
mock.onGet('/pages/knowledge-base').reply(config => {
  ...
  const filteredData = data.knowledgeBase.filter(
    obj => obj.title.toLowerCase().includes(queryLowered) || obj.desc.toLowerCase().includes(queryLowered)
  )
...
mock.onGet('/pages/knowledge-base/categories').reply(config => {
  ...
  Object.entries(data.knowledgeBaseCategory).forEach(entry => {
    ...
    const filteredQAndA = categoryObj.questions.filter(obj => obj.question.toLowerCase().includes(queryLowered))
