...
const schema = yup.object().shape({
  ...
  provence: yup.string().required('استان الزامی است'),
  ...
  floor: yup.string().required('طبقه الزامی است'),
  home_unit: yup.string().required('واحد الزامی است'),
...
function SidebarAddCourier({ open, toggle, setChange, user, edit, showUser }) {
  ...
  const onSubmit = data => {
    if (edit) {
      toast.promise(
        editUser(user.id, {
          address: `${data.main_street}- خیابان ${data.side_street} -کوچه ${data.alley} - پلاک ${data.plaque} - طبقه ${data.floor} - واحد ${data.home_unit}`,
          ...data,
          lat: LatLang[1],
          lang: LatLang[0]
        }).then(() => {
        ...
        {
          loading: 'در حال ویرایش بازاریاب',
          success: 'بازاریاب ویرایش شد',
      ...
      toast.promise(
        ...
        {
          loading: 'در حال ایجاد بازاریاب',
          success: 'بازاریاب ایجاد شد',
          error: err => (err?.response?.data?.message ? err.response?.data?.message : 'خطایی رخ داده است.')
        }
      )
    }
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <FormControl fullWidth sx={{ mb: 4 }}>
            ...
            />
            {errors.area_code && (
              <FormHelperText sx={{ color: 'error.main' }}>{errors.area_code.message}</FormHelperText>
            )}
          </FormControl>
          <FormControl fullWidth sx={{ mb: 4 }}>
            <Controller
