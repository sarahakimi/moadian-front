...
function ACLPage() {
  ...
  useEffect(() => {
    setLoading(true)
    http
      .get('customer/admin/1/all', sortModel, {
        Authorization: `Bearer ${window.localStorage.getItem('access_Token')}`
      })
      .then(async response => {
        setLoading(false)
  ...
  return (
    <Grid container spacing={6}>
      ...
      >
        <DialogTitle id='alert-dialog-title'>حذف نماینده</DialogTitle>
        <DialogContent>
          <DialogContentText id='alert-dialog-description'>نماینده با موفقیت حذف شد</DialogContentText>
        </DialogContent>
        <DialogActions>
